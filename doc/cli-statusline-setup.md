# CLI ステータスライン設定ガイド

Claude Code CLI でコンテキスト使用率を常時監視するための設定方法です。

> **注意**: この機能は **CLI専用** です。VSCode拡張では表示されません。
> VSCode拡張では `/status` コマンドで代替できます。

## 概要

コンテキスト使用率をステータスラインに常時表示することで：
- 現在のコンテキスト消費量を把握
- 使用率に応じた色分けで直感的に状況を認識
- 急増時に警告表示

## セットアップ手順

### 1. statusline.js を作成

`~/.claude/statusline.js` を作成します：

```javascript
// ~/.claude/statusline.js
// コンテキスト使用率表示スクリプト

let lastUsage = 0;
let lastTime = Date.now();

function getStatusLine(context) {
  const usage = context.tokenUsage?.percentUsed || 0;
  const currentTime = Date.now();

  // 急増検知（5秒以内に5%以上増加）
  const timeDiff = (currentTime - lastTime) / 1000;
  const usageDiff = usage - lastUsage;
  const isSpike = timeDiff < 5 && usageDiff > 5;

  lastUsage = usage;
  lastTime = currentTime;

  // 色分け
  let color;
  let status;
  if (usage <= 50) {
    color = 'green';
    status = '';
  } else if (usage <= 70) {
    color = 'yellow';
    status = ' [注意]';
  } else if (usage <= 85) {
    color = 'orange';
    status = ' [警告]';
  } else {
    color = 'red';
    status = ' [危険]';
  }

  // 急増警告
  const spikeWarning = isSpike ? ' ⚡急増!' : '';

  return {
    text: `📊 ${usage.toFixed(1)}%${status}${spikeWarning}`,
    color: color
  };
}

module.exports = { getStatusLine };
```

### 2. settings.json に設定を追加

`~/.claude/settings.json` に以下を追加：

```json
{
  "statusLine": {
    "enabled": true,
    "script": "~/.claude/statusline.js",
    "refreshInterval": 2000
  }
}
```

### 3. Claude Code を再起動

設定を反映するため、Claude Code CLI を再起動します。

## 表示内容

### 通常表示

```
📊 45.2%           ← 使用率50%以下（緑）
📊 62.8% [注意]    ← 使用率51-70%（黄）
📊 78.5% [警告]    ← 使用率71-85%（橙）
📊 92.1% [危険]    ← 使用率86%以上（赤）
```

### 急増検知

5秒以内に5%以上使用率が増加した場合：

```
📊 67.3% [注意] ⚡急増!
```

## 使用率の目安

| 使用率 | 状態 | 推奨アクション |
|--------|------|---------------|
| 0-50% | 良好 | そのまま作業継続 |
| 51-70% | 注意 | 大きなファイルの読み込みは計画的に |
| 71-85% | 警告 | タスク完了後 `/clear` を検討 |
| 86-100% | 危険 | 早めに `/clear` で新しいセッションを開始 |

## トラブルシューティング

### ステータスラインが表示されない

1. `~/.claude/settings.json` の構文エラーを確認
2. `statusline.js` のパスが正しいか確認
3. Claude Code を完全に再起動

### 色が表示されない

ターミナルが ANSI カラーをサポートしているか確認してください。

## 関連情報

- VSCode拡張ユーザー向け: `/status` コマンドで同等の情報を取得できます
- コンテキスト管理のベストプラクティス: [context-management.md](./context-management.md)
