...
>
> ```
> c:\repo\dd-know-how の doc/UPGRADE-NOTICE.md の v7 手順で更新して
> ```
>
> 呼び出し口（`scripts/dd-update.sh`）が入る2回目以降は、各プロジェクトで `bash scripts/dd-update.sh`（または「DDスキルを更新して」）だけで済む。本手順書が要るのは初回のみ。

## v7: プロセス簡素化 — 脱・自動レビュー（2026-07-28）

### 概要

自動多段レビュー（Phase 0 判定 → Phaseごとの DA・Codexレビュー）が時間コストに見合う品質改善を生んでいなかったため、**プロセスが強制するのは記録と機械検証だけ**に絞った:

1. **テンプレート軽量化**: Phase 0 の判定4点セットと Phaseごとの DA / Codex タスクを廃止。代わりに冒頭の**リスク判定1行**（認可・DB移行・外部I/F・機密情報に触れるか）と、**完了前チェック**（受け入れ基準照合・セルフレビュー1巡・全回帰1回）に集約
2. **レビューは人間が都度指示**: 実装は1モデル（Opus / Codex のどちらか）でDDを完走 → 完了報告 → 人間がリスク・規模を見て、必要なら**もう一方のモデル**にレビューを指示する。`codex-review.sh` はその実行手段として残る（自動実行はしない）
3. **da-method.md は参考文書に**: 深掘りが要る局面で使う手法集として保持（必須工程から除外）
4. **dd-auto スキル廃止**（下記）

> v6（セキュリティ堅牢化）が未取り込みのプロジェクトは、本手順の dd-update で **v6+v7 が一括で入る**。
...
bash scripts/dd-update.sh             # 取り込み
```

- テンプレ・スキル・方法論文書は**ローカル改変保護**（v6機構）の対象: 前回配布から未改変なら自動で新版になり、ローカル改変を検出したら「⚠️ 手動確認」リストに回る。既定配置のままのプロジェクトで確実に新版へ揃えるなら `bash scripts/dd-update.sh --adopt-content`
- 初回実行で `.dd-manifest`（改変判定用ハッシュ）が生成される → **git にコミット推奨**

**2. dd-auto スキルを使っていたプロジェクトのみ: 削除**

対象（2026-07 時点）: minecraft_creator / nanairo-workspace / spreadjs

```bash
rm -rf .claude/skills/dd-auto .agents/skills/dd-auto
grep -rn "dd-auto" AGENTS.md CLAUDE.md 2>/dev/null   # 言及があれば該当行を削除
```

DDの自動完走は「1モデルにDD単位で実行を指示する」通常運用に置き換わる（スキル不要）。
...
```bash
bash scripts/dd-health.sh        # エラーなく完走すること
grep -c "完了前チェック" {テンプレートフォルダ}/dd_template.md   # 1 が返れば新テンプレ
```

### 旧版プロジェクト（`scripts/dd-update.sh` が無い）の扱い

一斉移行は不要。**次にそのプロジェクトを触るタイミング**で dd-know-how から `/setup` を実行し直すのが最短（v7はファイル数・ルール量が減っているため、旧版との手動マージより再導入が速い）。それまで旧版はそのまま動き続ける。

### 既存DDへの影響

- **進行中のDD**: 旧構成（Phase 0 4点セット・DA記録表・Codexタスク入り）のまま完走してよい。残っている Codexレビュータスクは「レビューは人間指示に変更（v7）」とログ1行でスキップ可
- **新規DD**: 更新後のテンプレートで自動的に v7 構成になる
- **アーカイブ済みDD**: 変更不要。dd-health の DA関連指標は旧テンプレDD向けとして残っている

## v6: 配布フック/スクリプトのセキュリティ堅牢化（2026-07-09）

### 概要

利用先での自動セキュリティレビュー指摘（3件＋補足）に対応し、配布物のガードレールを堅牢化した。**セキュリティ修正のため、DDを導入済みの全プロジェクトで取り込み推奨**。

1. **`.dd-config` の任意コード実行を排除** — 従来は設定ファイルを `source`（＝中身をシェルとして実行）していたため、`.dd-config` に混入した任意コマンドが Bash 実行のたび・スクリプト実行のたびにユーザー権限で走り得た。`source` を廃し、必要キーだけを厳格抽出する方式に変更（`$( )` `;` 等を含む行は実行されず無視される）。
2. **pre-edit-guard の Windows バイパスを修正** — 実 JSON のエスケープされたバックスラッシュ（`C:\\...`）・大文字表記（`.CLAUDE`）・冗長パス（`./`）で `.claude/settings.json` 等の保護を素通りできた穴を、パス正規化（スラッシュ統一・重複畳み込み・小文字化）で封鎖。
3. **ガード保護対象を一式に一般化** — フック本体・`.codex`/`.agents` のフック・`.dd-config` を保護対象に追加（フックを書き換えてガード自体を無効化する経路を封鎖）。
...
これで `pre-edit-guard.sh` / `post-bash-dd-archive-reminder.sh` と各スクリプトが堅牢化版に置き換わる（差分のあるファイルだけ更新・コミットはしない）。

**2. Codex 用に `.codex/hooks/` を手動ミラーしている場合は再ミラー**

`.claude/hooks/` の各 `.sh` を `.codex/hooks/` へコピーし直す（byte-identical にする）。v6 から `dd-update.sh` は `.codex/hooks/` が存在すれば自動更新対象に含めるため、既に置いてあれば手順1でカバーされる。
...
- **`.dd-config` の書式は従来どおり** `KEY="value"`。ただし今後は「実行されるスクリプト」ではなく「厳格に読まれるデータ」になる。値に使える文字は英数・空白と `._:/~-` のみ（`SOURCE_REPO="C:/repo/dd-know-how"` のような Windows 絶対パスも可）。それ以外を含む行は無視されるので、既存の `.dd-config` に特殊文字を使っていないか一度確認するとよい。
- **ガードが `.dd-config` とフック本体の編集をブロックするようになる**。これらを書き換えたい場合は、hook 経由でなく直接エディタ/git で行う（設計意図どおり）。
- **残存リスク**（文字列マッチの限界）: シンボリックリンク経由の別名編集は防げない。また `file_path` を抽出できない稀なケースではフェイルオープン（通す）— 過剰ブロックによるロックアウトを避けるための設計判断。
- **dd-update がテンプレ/スキルのローカル改変を保護するようになった**: 非標準のドキュメント配置（例: `doc/process/da-method.md`）に合わせてテンプレやスキルのパス参照を調整している場合、従来の `dd-update` はそれを毎回上書きして壊していた。v6以降はコード（scripts/hooks）だけ常に上書きし、テンプレ/スキル/方法論文書はローカル改変を検出したら上書きせず「手動確認」リストに回す（判定用に `.dd-manifest` を生成。git にコミット推奨）。新版を全面採用したいときは `bash scripts/dd-update.sh --adopt-content`。
