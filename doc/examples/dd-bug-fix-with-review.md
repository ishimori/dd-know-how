# 実例: バグ修正DD（DA批判レビュー記録付き）

ハードコード値に起因するバグを修正したDDの実例です（匿名化済み）。
**DA批判レビュー（Devil's Advocate視点）**で実際に問題を発見した記録を含みます。
...
- [x] L782の目標値計算を修正
- [x] 😈 **DA批判レビュー（「このPhaseで何が壊れるか」を最低1件発見）**
...
各Phaseで **最低1件以上** の発見を記録。形式的な「問題なし」ではなく、「どこが壊れるか？」の視点で具体的な問題を探して記録している。
...
- Phase 1（小規模）: 1件発見
- Phase 2（中規模）: 2件発見 → 重要度「中」以上あり、別角度で再チェック実施
- Phase 3（中規模）: 2件発見 → 重要度「中」以上あり、別角度で再チェック実施

「発見ゼロは疑わしい」の原則に基づき、各Phaseで必ず問題を探している。
さらに重要度「中」以上の発見があれば、別角度での再チェックを発動する。
