...
### Changed
- **識別子付きDD（`DDK-001` 等）をスクリプト標準対応に**（利用先案件のローカル改変を上流還元。運用ガイド `doc/customization/parallel-sessions.md` 新設）
  - `templates/scripts/dd-index-gen.sh`: 収集 glob を `DD-*.md` → `DD*-*.md` に拡大し、dd-health.sh と同じ `^DD[A-Z]*-[0-9]` 判定で絞る — 識別子付きDDが索引から黙って欠落していた（ソートキーはもともと接頭辞対応済みで、収集だけが未対応）。付随して、番号を持たない `DD-メモ.md` のようなファイルは索引対象外になる。TS参考実装（`templates/scripts/README.md`）も同一条件に更新
  - `templates/scripts/dd-health.sh`: `--dd DDK-001` の識別子指定に対応（従来は接頭辞剥がしが `K-001` を残して検索不能 = 識別子付きDDは単体チェックが通らなかった）。識別子明示ならその系列に限定、番号のみなら素の `DD-` を優先して無ければ識別子付きへ広げる（`DD-001`/`DDK-001` 併存時の誤ヒット防止）。小文字入力の正規化と、エラーメッセージへの入力値そのまま表示も追加
  - `/dd` スキル: 識別子付き系列では**系列内の最大番号**から連番を取ることを明記（ミラー同期済み）
- **`git add -A` 禁止を正本ルール化**（利用先で他セッションの作業・gitignore 漏れの認証情報ファイルを巻き込む実事故があったため）
  - `templates/AGENTS.md.snippet`: コミット項を独立させ「stage は対象ファイルを明示する」を明記
  - `doc/UPGRADE-NOTICE.md`: v6/v7 手順内の `git add -A` 例を明示 stage に変更
...
- **DD運用の軽量化5点を正本テンプレ/方法論へ反映**（利用先 nanairo-workspace の DD-018 適用 → DD-019 実測「良好」を受けた上流反映。関連: nanairo DD-019 / DD-021）
  ...
  - `doc/da-method.md`: Codex併用時のPhase内DA省略ノート
  - 背景: 小規模変更にもフルプロセスが適用され、実行時間の大半がコード以外（全回帰の多重実行・DA/Codex二重レビュー・多段ゲート・記録肥大）に費やされていた（DD-010〜017 の工程分析）
- **dd-update のコンテンツ配布をローカル改変保護型に（`tools/dd-update-core.sh`）** — 従来はテンプレ/スキル/方法論文書も無条件上書きし、利用先がローカル配置に合わせて調整したパス参照（例: `doc/process/da-method.md`）を毎回破壊していた
  - 2層化: **コード**（`scripts/` ・ `hooks/`＝プロジェクト固有値を持たない）は従来どおり常に上書き。**コンテンツ**（テンプレ・スキル・方法論文書）は `put_safe` で保護
