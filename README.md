...
| 優先順 | アプローチ | 用途 | テンプレート |
|:------:|-----------|------|-------------|
| 1 | **バグ修正** | 既知のバグ修正（再現→修正→Before/After検証） | `dd_template.md` + `dd_template_bugfix.md`（差分） |
| 2 | **モック先行** | 画面の新規作成・大幅変更（見た目の合意が必要） | `dd_template.md` + `dd_template_mock.md`（差分） |
| 3 | **E2E駆動** | 画面の振る舞い中心（操作→結果の検証が必要） | `dd_template.md` + `dd_template_e2e.md`（差分） |
| 4 | **TDD** | 計算・遷移・ビジネスロジックが中心 | `dd_template.md` + `dd_template_tdd.md`（差分） |
| 5 | **標準（探索的実装）** | 上記以外 | `dd_template.md` のみ |
...
5つのアプローチ（標準/バグ修正/TDD/モック先行/E2E駆動）は、標準テンプレート（`dd_template.md`）を骨格とし、アプローチ固有のPhaseだけを差分テンプレートで差し替える方式です。

```
dd_template.md（共通骨格: 目的・検討・決定・タスク・ログ）
  + dd_template_bugfix.md → バグ修正のPhase構成に差し替え + セクション構成を置換
