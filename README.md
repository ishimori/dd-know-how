...
DA批判レビューの信号対雑音比（SNR）を高め、レビュー品質を維持します。詳細は [doc/da-method.md](doc/da-method.md) を参照。
...
| スキル | 役割 |
|--------|------|
| `/dd` | DD操作（作成・参照・一覧・検索・アーカイブ）。DA批判レビューはDDテンプレートに組み込み済み |
...
あるフルスタックプロジェクト（React + Firebase + 外部API連携）で DD-Know-How を継続運用した実績です。
...
| 種類 | 例 | 件数帯 |
|------|-----|-------|
| 機能実装 | 画面追加、CRUD実装、認証フロー | 最多 |
| アーキテクチャ | API負荷分散設計（キューイング）、DB設計 | 5件+ |
| インフラ | 環境変数整理、DB移行、セキュリティ整備 | 5件+ |
| バグ修正 | ハードコード修正、ロジック修正 | 10件+ |
| 管理DD | 親DD→サブDD分割で大規模開発を統括 | 3件+ |

### 管理DDパターン: 大規模開発の実例

27画面のReact実装を1つの管理DDで統括し、作業項目ごとにサブDDを発行:

```
DD-056: React画面実装（管理DD）
├── DD-056-1: 共通基盤         ✅ 完了
├── DD-056-2: デザインガイド    ✅ 完了
├── DD-056-3: 認証フロー        ✅ 完了
├── DD-056-4: ホーム画面        ✅ 完了
...
管理DDには全体のロードマップ・依存関係・非機能要件を記載し、各サブDDは具体的な設計・実装・DAレビューを担う。完了したサブDDの成果（ビルドサイズ、DA発見数）は管理DDのログに集約される。
...
いずれも「問題ありません」で素通りしていれば見落としていた事項です。
...
DDスキルは当初428行の単一ファイルでした。その後 DD 操作とワークフロー管理に分離しましたが、実運用では `/workflow` の出番がほとんどなく、DA批判レビューはDDテンプレートのタスク項目で十分に機能していました。

現在は `/dd` スキル + DA メソッド文書（`doc/da-method.md`）の構成に統合し、シンプルさと品質の両立を実現しています。
