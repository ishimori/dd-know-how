...
DA批判レビューは同一会話コンテキスト内で実行されるため、構造的に楽観バイアスの影響を完全には排除できません。GEM分析はこの限界を補完します。

**DDの最終Phase完了後に、独立した視点から DD全体をレビュー:**

| モード | 条件 | 効果 |
|--------|------|------|
| Gemini API | `.env` に `GEMINI_API_KEY` を設定 | 異なるモデルによる完全に独立したレビュー |
| 別コンテキストDA | API キー未設定 | 会話コンテキストを排除した別スレッドでのDA分析 |

```
各Phase完了 → DA批判レビュー（自己チェック）
                    ↓
最終Phase完了 → ★ GEM分析（独立レビュー）
                    ↓
Step 8（仕様書同期）→ Step 9（コミット）
```

設定方法は [IMPORT.md](IMPORT.md) を参照。
