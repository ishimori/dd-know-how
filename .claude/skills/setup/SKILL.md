...
│   └── skills/
│       └── dd/
│           └── SKILL.md      # ← .claude/skills/dd/SKILL.md（正本）
├── .agents/
│   └── skills/
│       └── dd/
...
├── AGENTS.md                 # ← templates/AGENTS.md.snippet をベースに作成（指示ファイルの正本。ステップ6）
└── CLAUDE.md                 # ← templates/CLAUDE.md.snippet（@AGENTS.md ポインタ。ステップ6）
```

### 5. Hooks の設定（Level 2 のみ）

> Hooks は Claude Code 固有の機構（Codex には無い）。Codex のみで使うプロジェクトではこのステップをスキップしてよい。
...
### 6. AGENTS.md（正本）と CLAUDE.md（ポインタ）の作成・更新

指示ファイルは **AGENTS.md が正本**（Codex は直接、Claude Code は CLAUDE.md の `@AGENTS.md` インポート経由で読む）。**同じ内容を2ファイルに書かないこと。**

**AGENTS.md** — `templates/AGENTS.md.snippet` をベースに作成する。**全体を50行程度に収めること**（全セッションのコンテキストに常駐するため、詳細は doc/ 配下に置き、AGENTS.md はポインタ集に徹する）。
...
- **存在しない場合**: `templates/CLAUDE.md.snippet` をそのままコピー（中身は `@AGENTS.md` ポインタのみ。プレースホルダなし・編集不要）
- **存在する場合（既存プロジェクトに内容入りの CLAUDE.md がある）**: 冒頭に `@AGENTS.md` の1行を追加する（既にあればスキップ）。DD関連セクションは CLAUDE.md ではなく AGENTS.md 側へ追記する。既存の汎用的な内容（技術スタック・コマンド等）の AGENTS.md への移動は提案にとどめ、実施はユーザーの合意を得てから
...
| 10 | CLAUDE.md | 冒頭に `@AGENTS.md` インポート行があるか |
| 11 | `.agents/skills/dd/` | `SKILL.md` が `.claude/skills/dd/SKILL.md` と同一内容か（Level 2。`diff` で確認） |
| 12 | `.dd-config`（ルート直下） | 存在し、`DD_DIR` / `ARCHIVE_DIR` の指す実フォルダが存在するか（Level 2） |
...
✓ パス整合性チェック:
  ...
  CLAUDE.md             → @AGENTS.md インポート行あり     ✓ 確認
  ...
  .agents/skills/dd     → .claude/skills/dd と同一内容      ✓ diff確認 (Level 2)
...
- スキル: .claude/skills/dd/ + .agents/skills/dd/（同一内容。Level 2以上）
- AGENTS.md: {作成 or 追記}（指示ファイルの正本・50行程度）
- CLAUDE.md: {作成 or @AGENTS.md 追記}（ポインタ）
...
| スキル | `.claude/skills/`（Claude Code）+ `.agents/skills/`（Codex・同一内容） | （固定） |
| フック | `.claude/hooks/` | （固定・Claude Code 専用） |
| スクリプト | `scripts/` | （任意） |
...
- 既存ファイルがある場合は上書き前に確認
- AGENTS.md / CLAUDE.md への追記は重複チェックを行う
