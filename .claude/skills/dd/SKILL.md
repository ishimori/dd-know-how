...
1. **必ず最初に `templates/guides.md` を読み、アプローチと使用するテンプレートを決定する**
2. DDフォルダとアーカイブから最大番号を取得
3. 次の連番でファイル作成: `DD-{連番}_{タイトル}.md`
4. 決定したテンプレートを使用（`{番号}` `{タイトル}` `{日付}` を置換）
5. **出力するDDにテンプレートのHTMLコメント（`<!-- ... -->`）を残さないこと**
